...
could try running [prettier-check], which is a tool that checks if your code is
formatted the way prettier formats it. Try it by running:

```
npm install prettier prettier-check
git rebase --exec 'prettier-check --single-quote exercise-2/calc.js' HEAD~3
```

Try to introduce an error in an earlier commit, or run prettier-check without
the `--single-quote` option to see what happens when the check fails.

[prettier-check]: https://github.com/hexacta/prettier-check
